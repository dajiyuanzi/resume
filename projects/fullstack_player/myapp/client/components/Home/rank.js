...
class Ranking extends Component {
  ...
  componentDidMount() {
    this.props.dispatch(officialRankDetail()); //官方榜
    this.props.dispatch(nationalRankDetail()); //全球榜
  }

  render() {
    const { officialRank } = this.props;
    const { nationalRank } = this.props;
    return (
...
function select(state) {
  return {
    officialRank:state.rank.officialRank, //rank是来自reducers/rootReducer.js
